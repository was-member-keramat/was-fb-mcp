...
import { fileURLToPath, pathToURL } from 'node:url';
...
// EVERY dynamic import() MUST be wrapped in pathToURL(...).href
// Otherwise Windows Node 24+ throws ERR_UNSUPPORTED_ESM_URL_SCHEME

if (cmd === 'auth') {
  const { runAuthFlow } = await import(pathToURL(join(__dirname, 'auth.js')).href);
  try {
    await runAuthFlow();
    process.exit(0);
  } catch (err) {
  ...
  const { deleteConfigFile } = await import(pathToURL(join(__dirname, 'auth.js')).href);
  const deleted = await deleteConfigFile();
  console.log(deleted ? `Removed ${CONFIG_FILE}` : 'No saved credentials to remove.');
  process.exit(0);
} else if (cmd === 'status') {
  const { readConfigFile } = await import(pathToURL(join(__dirname, 'auth.js')).href);
...
`);
  ...
  await import(pathToURL(join(__dirname, 'server.js')).href);
}
