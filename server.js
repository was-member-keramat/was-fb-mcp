  ...
  return {
    ...
    content: [{
      ...
      text: 'Facebook / Meta Graph API error:\n' + JSON.stringify({
        ...
        error_subcode: vendorError?.error_subode ?? null,
...
const tools = [
  ...
  },
  {
    ...
    inputSchema: {
      type: 'object',
      properties: {
        pageId: { type: 'string', description: 'Facebook Page IC' },
  ...
  {
    name: 'fb_list_pixels',
    description: 'List Meta Pixels and Datasets for an Ad Account.',
    inputSchema: {
      type: 'object',
      properties: {
        adAccountId: { type: 'string', description: 'Ad Account ID (e.g. "act_123456789")' },
        fields: { type: 'string', description: 'Comma-separated fields to request (default: "id,name,code,creation_time,last_fired_time,is_created_by_business")' }
      },
      required: ['adAccountId']
    }
  },
  {
    name: 'fb_create_pixel',
    description: 'Create a new Meta Pixel or Dataset under an Ad Account.',
    inputSchema: {
      type: 'object',
      properties: {
        adAccountId: { type: 'string', description: 'Ad Account ID (e.g. "act_123456789")' },
        name: { type: 'string', description: 'Name for the new Pixel or Dataset' }
      },
      required: ['adAccountId', 'name']
    }
  },
  {
    name: 'fb_update_pixel',
    description: 'Update / Edit an existing Meta Pixel or Dataset.',
    inputSchema: {
      type: 'object',
      properties: {
        pixelId: { type: 'string', description: 'Pixel ID or Dataset IC' },
        name: { type: 'string', description: 'New name for the Pixel or Dataset' }
      },
      required: ['pixelId']
    }
  },
  {
    name: 'fb_delete_pixel',
    description: 'Delete or remove a Meta Pixel or Dataset.',
    inputSchema: {
      type: 'object',
      properties: {
        pixelId: { type: 'string', description: 'Pixel ID or Dataset ID to delete' }
      },
      required: ['pixelId']
    }
  },
  {
    name: 'fb_generate_long_lived_token',
    description: 'Exchange a short-lived Meta user access token for a 60-day long-lived access token.',
    inputSchema: {
      type: 'object',
      properties: {
        appId: { type: 'string', description: 'Meta App ID (Client ID)' },
        appSecret: { type: 'string', description: 'Meta App Secret (Client Secret)' },
        shortLivedToken: { type: 'string', description: 'Optional Short-lived User Access Token (defaults to active token)' }
      },
      required: ['appId', 'appSecret']
    }
  },
  {
    name: 'fb_get_page_access_token',
    description: 'Generate / Retrieve a Page Access Token for a specific Facebook Page.',
    inputSchema: {
      type: 'object',
      properties: {
        pageId: { type: 'string', description: 'Facebook Page ID' }
  ...
  {
    name: 'fb_debug_token',
    description: 'Inspect and debug a Meta Access Token to check expiration, validity, app ID, user ID, and granted permissions/scopes.',
    inputSchema: {
      type: 'object',
      properties: {
        inputToken: { type: 'string', description: 'Access Token to inspect/debug (defaults to active token)' }
      }
    }
  }
];
